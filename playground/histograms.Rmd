---
title: "Indices of ecological networks in Web of Life"
...
## Network size 
```{r}
ggplot(df, aes(x=(num_resources+num_consumers), color=network_type, fill=network_type)) +
...
```{r}
df_1 <- filter(df, ((num_resources+num_consumers) >30) & ((num_resources+num_consumers) <100))
ggplot(df_1, aes(x=(num_resources+num_consumers), color=network_type, fill=network_type)) +
  geom_density(alpha = 0.2) + 
  xlab("network size") + 
  xlim(30, 100)  # alpha is the fill transparency
```
...
```{r}
ggplot(df, aes(x=network_size_ratio, color=network_type, fill=network_type)) +
  geom_density(alpha = 0.2) + xlab("resources/consumers")
```

## Connectance
```{r} 
ggplot(df, aes(x=connectance, color=network_type, fill=network_type)) +
  geom_density(alpha = 0.2) + xlim(0, 1)  # alpha is the fill transparency
```

**filtered by network size**

```{r} 
ggplot(df_1, aes(x=connectance, color=network_type, fill=network_type)) +
  geom_density(alpha = 0.2) + xlim(0, 1)  # alpha is the fill transparency
```
...
## Nestedness 
```{r} 
ggplot(df, aes(x=nestedness_bascompte, color=network_type, fill=network_type)) +
...
```{r} 
ggplot(df_1, aes(x=nestedness_bascompte, color=network_type, fill=network_type)) +
  geom_density(alpha = 0.2) + xlim(0, 1) 
```


```{r} 
ggplot(df, aes(x=nestedness_weighted, color=network_type, fill=network_type)) +
  geom_density(alpha = 0.2) + xlim(0, 1) 
```

```{r} 
ggplot(df, aes(x=nestedness_temperature, color=network_type, fill=network_type)) +
  geom_density(alpha = 0.2) 
```
...
```{r} 
ggplot(df_1, aes(x=nestedness_temperature, color=network_type, fill=network_type)) +
  geom_density(alpha = 0.2) 
```
...
```{r} 
ggplot(df, aes(x=modularity_fast_greedy, color=network_type, fill=network_type)) +
  geom_density(alpha = 0.2) + xlim(0, 1)  # alpha is the fill transparency
```
...
```{r} 
ggplot(df_1, aes(x=modularity_fast_greedy, color=network_type, fill=network_type)) +
  geom_density(alpha = 0.2) + xlim(0, 1)  # alpha is the fill transparency
```
...
```{r} 
ggplot(df, aes(x=modularity_edge_betweenness, color=network_type, fill=network_type)) +
  geom_density(alpha = 0.2) + xlim(0, 1)  
```
...
```{r} 
ggplot(df_1, aes(x=modularity_edge_betweenness, color=network_type, fill=network_type)) +
  geom_density(alpha = 0.2) + xlim(0, 1)  
```
...
```{r} 
ggplot(df, aes(x=modularity_leading_eigen, color=network_type, fill=network_type)) +
  geom_density(alpha = 0.2) + xlim(0, 1)  
```
...
```{r} 
ggplot(df_1, aes(x=modularity_leading_eigen, color=network_type, fill=network_type)) +
  geom_density(alpha = 0.2) + xlim(0, 1)  
```
...
The qualitative behavior should be the opposite w.r.t. `nestedness_bascompte` because $T=0$ corresponds ot a perfectly nested network (**Achtung**: not observed!). 
