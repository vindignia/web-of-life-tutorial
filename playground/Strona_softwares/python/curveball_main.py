...
def read_input_w_no_delimiters(csvfile):
    #Ale
    print("in read_input_w_no_delimiters()")
...
def read_matrix_from(dat_file, delimiter=None):
    ...
    with open(dat_file, 'r') as csvfile:
        try:
            if not delimiter:
...
def add_headers(col_names, out_mat, row_names):
    ...
    if col_names and len(col_names) > 0:
        ...
        # keep only the data and not the header with column names
        out_mat = np.array(temp_mat[1:], dtype='|S4').reshape((orig_shape[0], orig_shape[1]))


    if row_names is not None:
        #must modify the txt numpy file to add row and column names
        #as in the original input matrix
...
def generate_matrix(col_names, delimiter, mat, options, presences, row_names):
    ...
    for i in range(options.num_matrices):
        ...
        out_mat_int = np.array([[int(chr(item[0])) for item in row] for row in out_mat])
        file_out = os.path.join(options.output_folder, 'out_mat_' + str(i + 1) + '.txt')
        np.savetxt(file_out, out_mat_int, delimiter=delimiter, fmt=fmt_)
...
def main():
    ...
    parser.add_option("-o", "--output-folder",
                      ...
                      help="Folder where random null matrices will be saved")
    ...
    #this method will iterate to produce the desired number of null matrices
